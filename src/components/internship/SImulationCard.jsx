  ...
  return (
    <div className="group bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-lg hover:border-indigo-200 transition-all duration-200">
      <Link to={`/simulation/${id}`}>
        <div className="relative h-40 bg-gradient-to-br from-blue-600 to-violet-600 flex items-center justify-center overflow-hidden">
          {image ? (
            <img
              src={image}
              alt={title}
      ...
      <div className="p-5">
        {/* Category Tag */}
        {category && (
          <span className="inline-block px-2.5 py-1 text-xs font-medium text-blue-700 bg-blue-50 rounded-md mb-3">
            {category}
          </span>
        )}

        {/* Title */}
        <h3 className="text-base font-semibold text-slate-900 mb-1 group-hover:text-blue-700 transition-colors line-clamp-2">
          {title}
        </h3>
